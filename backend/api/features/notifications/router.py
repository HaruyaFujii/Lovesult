...
async def get_notifications(
    ...
    notifications, next_cursor = await usecase.get_notifications(current_user_id, cursor, limit)
    return NotificationListResponse(notifications=notifications, next_cursor=next_cursor)


@router.get(
    "/unread-count", response_model=UnreadCountResponse, operation_id="getUnreadNotificationCount"
)
